...
#elif defined(USE_ARMCLZ)

#define ARMCLZ(a) \
   ({unsigned __value, __arg = (a); asm("clz\t%0, %1": "=r" (__value): "r" (__arg)); __value; })

inline int armclz(unsigned i) { return ARMCLZ(i); }

inline int armclz64(uint64_t b) {
   unsigned u = (unsigned)(b>>32);
   return u? armclz(u) : (32+armclz((unsigned)b));
}

inline Square first_1(Bitboard b) {
   return Square(armclz64(b) ^ 63);
}
