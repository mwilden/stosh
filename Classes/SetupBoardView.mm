...
- (void)putPiece:(Piece)p onSquare:(Square)s {
   if (board[s] == EMPTY) {
      ...
      UIImageView *iv =
         [[UIImageView alloc]
            initWithFrame: CGRectMake(int(s)%8 * 40.0f, (7-int(s)/8) * 40.0f,
                                      40.0f, 40.0f)];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
   if (phase == PHASE_EDIT_BOARD) {
      ...
      [selectedSquareView moveToPoint:
                             CGPointMake(column*40.0f - 30.0f, row*40.0f - 30.0f)];
      ...
      for (i = 0; epSquares[i] != SQ_NONE; i++)
         if (epSquares[i] == selectedSquare) {
            [selectedSquareView moveToPoint:
                                   CGPointMake(column*40.0f - 30.0f,
                                               row*40.0f - 30.0f)];
            return;
         }
      selectedSquare = SQ_NONE;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
   if (phase == PHASE_EDIT_BOARD && selectedSquare != SQ_NONE) {
      if (board[selectedSquare] == EMPTY) {
         ...
         CGFloat x = (int)square_file(selectedSquare) * 40.0f;
         CGFloat y = (int)(7 - square_rank(selectedSquare)) * 40.0f;
         CGRect r;
