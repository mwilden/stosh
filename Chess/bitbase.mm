/*
  ...
  Copyright (C) 2004-2009 Tord Romstad
...
void generate_kpk_bitbase(uint8_t bitbase[]) {
  ...
  // Look for a kpk.bin file on disk, and read it into memory if found:
  f = fopen([[[NSBundle mainBundle] pathForResource: @"kpk"
                                    ofType: @"bin"]
              UTF8String],
            "rb");
  if(f != NULL) {
    printf("Reading bitbase from file\n");
    for(int i = 0; i < 24576; i++) bitbase[i] = fgetc(f);
    fclose(f);
    return;
  }

  // No bitbase found, generate it instead:
  printf("Generating bitbase\n");
  ...
  else printf("Failed to open file for saving bitbase\n");
}
